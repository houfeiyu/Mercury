...
function expose() {
    var t = window.haoutil;
    haoutil.noConflict = function () {
        return window.haoutil = t, this
    }, window.haoutil = haoutil
}

var haoutil = haoutil || {};
haoutil.version = "2.2", haoutil.msg = function (t) {
    layer.msg(t)
}, haoutil.tip = haoutil.msg, haoutil.alert = function (t, e) {
    layer.alert(t, {title: e || "提示", skin: "layui-layer-lan", closeBtn: 0, anim: 0})
}, haoutil.loading = {
    index: -1, show: function (t) {
        t = t || {style: 2, shade: [.3, "#000000"]}, this.index = layer.load(t.style, t)
    }, hide: function () {
        this.close()
    }, close: function () {
        layer.close(this.index)
    }
}, "object" == typeof module && "object" == typeof module.exports ? module.exports = haoutil : "function" == typeof define && define.amd && define(haoutil), "undefined" != typeof window && expose(), Array.prototype.indexOf = function (t) {
    ...
    /(y+)/.test(t) && (t = t.replace(RegExp.$1, (this.getFullYear() + "").substr(4 - RegExp.$1.length))), /(E+)/.test(t) && (t = t.replace(RegExp.$1, (RegExp.$1.length > 1 ? RegExp.$1.length > 2 ? "星期" : "周" : "") + n[this.getDay() + ""]));
    ...
    function r(t) {
        if (o && null != window.plus) plus.navigator.removeCookie(t); else {
            var e = new Date;
    ...
    var o;
    return {isH5Mobile: t, add: e, get: n, del: r}
    ...
    function n(e, n) {
        var o = n.toDataURL("image/png"), i = r(o);
        t(e + ".png", i)
    }

    function r(t) {
        for (var e = t.split(";base64,"), n = e[0].split(":")[1], r = window.atob(e[1]), o = r.length, i = new Uint8Array(o), u = 0; u < o; ++u) i[u] = r.charCodeAt(u);
        return new Blob([i], {type: n})
    }

    return {download: t, downloadFile: e, downloadImage: n, base64Img2Blob: r}
    ...
    return {isNull: u, isNotNull: a, isArray: t, isString: e, isNumber: n, isDate: r, isFunction: o, isObject: i}
    ...
    function r(t, e) {
        t = String(t);
        for (var e = t.length; e < n;) t = "0" + t, e++;
        return t
    }

    return {getArrayRandomOne: e, random: t, padLeft0: r}
}(), haoutil.storage = function () {
    function t(t, e) {
        r = window.localStorage, r.setItem(t, e)
    }
    ...
    function n(t) {
        r = window.localStorage, r.removeItem(t)
    }

    var r;
    return {add: t, get: e, del: n}
    ...
    function n(t) {
        return Number(t), t < 1e6 ? t.toFixed(2) + "平方米" : (t / 1e6).toFixed(2) + "平方公里"
    }

    function r(t) {
        return Number(t), t < 60 ? t + "秒" : t >= 60 && t < 3600 ? (t / 60).toFixed(0) + "分钟" : (t / 3600).toFixed(1) + "小时"
    }

    function o(t) {
        for (var e = "", n = t.length % 6, r = t.substr(0, t.length - n), o = t.substr(t.length - n, n), i = 0; i < r.length; i += 6) {
            var u = parseInt(r.substr(i, 6), 2);
            e += l[u]
        }
        return o += new Array(7 - n).join("0"), n && (e += l[parseInt(o, 2)], e += new Array((6 - n) / 2 + 1).join("=")), e
    }

    function i(t) {
        for (var e = "", n = 0, r = 0; r < t.length; r++) if ("=" != t[r]) {
            var o = l.indexOf(t[r]).toString(2);
    ...
    return {isChinese: t, formatLength: e, formatArea: n, formatTime: r, base64: c, decodeBase64: s}
    ...
    function e(t, e) {
        var n = new RegExp("(^|&)" + t + "=([^&]*)(&|$)", "i"), r = window.location.search.substr(1).match(n);
    ...
    function r() {
        ...
        if (t.indexOf("msie") >= 0) {
            ...
            return {type: "IE", version: e}
        }
        if (t.indexOf("firefox") >= 0) {
            var e = Number(t.match(/firefox\/([\d]+)/)[1]);
            return {type: "Firefox", version: e}
        }
        if (t.indexOf("chrome") >= 0) {
            var e = Number(t.match(/chrome\/([\d]+)/)[1]);
            return {type: "Chrome", version: e}
        }
        if (t.indexOf("opera") >= 0) {
            var e = Number(t.match(/opera.([\d]+)/)[1]);
            return {type: "Opera", version: e}
        }
        if (t.indexOf("Safari") >= 0) {
            var e = Number(t.match(/version\/([\d]+)/)[1]);
            return {type: "Safari", version: e}
        }
        return {type: t, version: -1}
    }

    function o() {
        var t = navigator.userAgent.toLowerCase(), e = "ipad" == t.match(/ipad/i), n = "iphone" == t.match(/iphone/i),
            r = "midp" == t.match(/midp/i), o = "rv:1.2.3.4" == t.match(/rv:1.2.3.4/i),
            i = "ucweb" == t.match(/ucweb/i), u = "android" == t.match(/android/i),
            a = "windows ce" == t.match(/windows ce/i), c = "windows mobile" == t.match(/windows mobile/i);
        return !(e || n || r || o || i || u || a || c)
    }
    ...
    function u(t, e, n) {
        var r = function (t, e, n) {
            var r = Math.random().toString().replace(".", ""), o = "my_json_cb_" + r;
            window[o] = n;
            var i = t.indexOf("?") == -1 ? "?" : "&";
            for (var u in e) i += u + "=" + e[u] + "&";
            i += "callback=" + o;
            var a = document.createElement("script");
            a.src = t + i, document.body.appendChild(a)
        };
        window.$jsonp = r
    }
